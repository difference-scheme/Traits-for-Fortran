...
type PairwiseSum[T INumeric, U ISum[T]] struct {
   other U
}

func (self PairwiseSum[T,U]) sum(x []T) T {
   if len(x) <= 2 {
...
type Averager[T INumeric, U ISum[T]] struct {
   drv U
}

func (self Averager[T,U]) average(x []T) T {
