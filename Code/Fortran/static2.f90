...
module pairwise_library
   ...
   type, public, implements(ISum) :: PairwiseSum{ISum :: U}
      private
      type(U) :: other
   contains
      procedure :: sum
...
module averager_library
   ...
   type, public, implements(IAverager) :: Averager{ISum :: U}
      private
      type(U) :: drv
   contains
      procedure :: average
...
contains
   ...
   function average{INumeric :: T}(self,x) result(a)
      class(Averager{U}), intent(in) :: self
      type(T),            intent(in) :: x(:)
      type(T)                        :: a
...
program main
   ...
   ! use of parameterized structure constructors for Averager
