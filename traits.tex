...
\section{Interfaces with generic procedure signatures}
...
are built into the language, and without having to rely exclusively on
external library functionality for this purpose.

For instance, generic routines will often have to initialize the
result of reduction operations. As it is the case in the test problem
of Sect.~\ref{}, where a variable for summation needs to be
initialized to the zero constant of type \code{T}. In Go, it is easily
possible to express this initialization by simply transforming the
zero constant of the \code{int} type, \code{0}, into the corresponding
constant of type \code{T} as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
   s = T(0)
\end{lstlisting}
...
also for all other cases where conversion is indeed
possible. Otherwise, the compiler should emit an error, and abort the
compilation at the generics instantiation step.
...
implemented in both traits and protocols in Rust and Swift,
respectively. In addition to different array ranks and array
...
non-essential, and we thus prefer to focus on them in a separate
document.
...
statically, as opposed to dynamically. This will enable polymorphic
method inlining by the compiler, to potentially improve code
...
(and hence static dispatch of their methods, as opposed to the
\code{class} specifier that was used previously in order to effect
run-time polymorphism and dynamic dispatch. See also
...
\lstinputlisting[language=LFortran,style=boxed,label={lst:staticFortran},caption={Demonstrates static method dispatch for the \code{sum} methods.}]{Code/Fortran/static1.ft}
...
As in a corresponding Swift code version of the test problem, the
instantiation of the objects \code{other} and \code{drv}
