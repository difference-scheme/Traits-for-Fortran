...
of being an implementor of \code{IBasicMath}, is now also an
implementor of \code{IAddition}, and \code{ISubtraction}. Hence,
...
The same algorithm as that of Sect.~\ref{sect:generic_procedures}, when
implemented as a generic method \code{sum} that is bound to a derived
type named \code{SimpleSum}, which implements the interface \code{ISum}
as it was given in Sect.~\ref{sect:generic_interfaces}, would instead
look as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
...
implemented by another derived-type, named \code{PairwiseSum}:
\begin{lstlisting}[language=LFortran,style=boxed]
...
Notice also, how \code{PairwiseSum}'s implementation of method
\code{sum} has access to the parameter \code{U} through the
declaration of its \code{self} object, that is declared being of
\code{class(PairwiseSum\{U\})}. Hence, this method can now, in
principle, make use of two independently defined generic type
parameters, \code{T} and \code{U}. This provides increased
flexibility.
...
would be fully backwards compatible. The elegance of the aforegiven Go
and Swift code versions, but also of the Fortran code examples that
