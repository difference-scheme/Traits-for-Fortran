...
settings. Go thus implements a ``true trait system''. Go, finally,
makes it explicit in its type definition syntax that interfaces (like
...
the first mainstream language that came with a ``trait system'', that
lets the user implement poylmorphic interfaces/traits for user-defined
types (e.g. \code{struct}'s) without such implementations having to be
coupled to the type's actual definition. In contrast to Go, Rust
employs an explicit way of doing this, namely by using explicit
``\code{impl}'' code blocks for providing a trait's method
implementations. These same \code{impl} blocks also serve to bind
methods to a type that aren't a part of some trait, like e.g.
user-defined constructors for \code{struct}s (the functions named
``\code{new}'' in the following code Listing~\ref{}).
...
polymorphic interfaces. These are called ``protocols'' in Swift. If
the Swift programmer chooses to ignore implementation inheritance and
...
``extension'' blocks (that moreover work even with implementation
inheritance).
...
programmer, that will be discussed in detail below.
...
Java). All of these languages thereby collectively rebut Haveraaen's
\cite{Haveraaen_et_al_19} critique of Java, namely to base
polymorphism on interfaces and interface inheritance. In fact, it is
...
first place in these modern languages, that Haveraaen seems to also
(rightly) demand. Given their many similarities, we will thus only
make some final comments on the differences in these languages'
...
discussing some further extensions that are exclusively required in
