...
purpose is to allow the programmer to declare polymorphic variables in
terms of them. Either directly, i.e. as objects of such interfaces in
run-time polymorphism, or indirectly as constraints on generic type
...
functions that work with all the member types of the type set. This
can also be restated, by saying that a type \code{T} \emph{implements}
an interface consisting of such a type set, if (and only if) it is a
member of this set. For instance, Fortran's various \code{complex}
types are not members of interface \code{INumeric}'s type set (as it
is given in this section), because they do not appear within its
definition. Hence, none of the \code{complex} types implements the
\code{INumeric} interface. In particular, the \code{complex} types do
...
regular arguments list, e.g. as in the following two examples:
\begin{lstlisting}[language=LFortran,style=boxed]
...
generics features seamlessly and easily within a modern-day OO programming
setting is one of the great strengths of the present design. The equal
