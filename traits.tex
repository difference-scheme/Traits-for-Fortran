...
that are contained in a language intrinsic module. The actual code of
these interfaces could then employ the ``interfaces-as-type-sets''
syntax that was described above, or any other suitable means. For
instance, a more general \code{INumeric} interface than the one given
above, could be coded as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: INumeric
...
Importantly, the language should provide interfaces for all its
numeric and relational operators. Both, for the user to formulate
generics constraints in terms of them for intrinsic types. But also in
order to enable the programmer to implement these operators himself
for any new types where this would be desirable, or where these
operators are not supported by default.
...
flexibility. It also means that there is \emph{no} implicit
...
different actual arguments for the generic parameter \code{U}. Hence,
...
with other modern languages. We believe that this will completely
transform the way Fortran applications will be written in the
...
will become possible, without any need for run-time type inspection,
and without any loss in computational performance.
