...
Our basic algorithm for caclulating the average value of an array of
...
depends on function \code{simple\_sum}'s implementation. While the
functions \code{simple\_average} and \code{pairwise\_average} depend
on the implementations of the functions \code{simple\_sum}, and
...
the \code{int} data type to declare the arrays that it is operating
on, as well as the results of the summations. Hence, this code cannot
be used on arrays of any other data type than \code{int}s.
...
functions whose signatures are contained in an interface. In Go,
there is no explicit statement for the purpose of implementing
interfaces. Instead an ADT is implicitly assumed to implement an
...
express polymorphism for the various objects of the \code{SimpleSum},
\code{PairwiseSum}, and \code{Averager} ADTs that implement either one
of these interfaces. The interface \code{INumeric} serves the same
purpose as in the functional polymorphic version given in
...
output parameters, respectively, both the \code{int} and
\code{float64} intrinsic types.

All the interfaces, except \code{INumeric}, and all the user-defined
ADTs take generic parameters in order to enable generics polymorphism
for the \code{sum} and \code{average} methods. The main program makes
use of Go's built-in structure constructors in order to instantiate
objects of all the ADTs. In particular, it instantiates run-time
polymorphic ``averager objects'' (depending on whether simple or
pairwise sum averaging is to take place), and it does so for both the
...
fact that \code{two} such objects are required (one for each data
type) is connected to the fact that in order to obtain generic methods
in Go, one has to parametrize interfaces by generic parameters, and
instantiate them with different data types. A single
(i.e. unparameterized) interface therefore doesn't suffice, which is
unfortunate from the user's perspective.
...
\begin{itemize}
  ...
  \item
    ...
    conversion).
  \item
    Notice, how in contrast to both Go and Rust, the Swift code
    doesn't instantiate any interfaces/protocols. For instance, there
    is only a single, unique version of the \code{IAverager}
    interface, and hence there's only a need in the main program to
    declare a single variable, \code{av}, of that interface (\code{av}
    being polymorphic over different \code{Averager} types). This
    single object can then be straightforwardly used on data of
    \emph{both} the \code{Int} and \code{Float64} types!
\end{itemize}
...
required changes as compared to Listing~\ref{lst:OOFortran} to effect
static dispatch of the \code{sum} methods.

The changes are confined to a parametrization of the
...
longer any such dynamic instantiation necessary for these
object. Instead, the unparameterized structure constructor of
\code{SimpleSum}, and the parameterized structure constructor of
\code{PairwiseSum} are called from the main program, in order to
instantiate these object statically.
...
Notice, that in Listing~\ref{lst:staticFortran}, the \code{av} object
of \code{IAverager} type that is initialized in the \code{select case}
statement of the main program still needs to make use of run-time
polymorphism. This object cannot be made to employ compile-time
polymorphism, as it is used within a statement that performs a
run-time decision.
