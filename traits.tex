...
The semantics of such a type union construct is that it implicitly
...
interface consisting of such a type union, if (and only if) it is a
member of this union. The \code{complex} type, for instance, is not a
member of interface \code{INumeric}'s type union (as it is given in
...
\end{lstlisting}
Notice how this makes use of both type unions, and kind parameters
...
types \cite{Taylor_21} (which would ultimately take this design to its
logical conclusion, provide an alternative to generics, and fill a
present gap in the later to be discussed Table~\ref{tab:dispatch}).
