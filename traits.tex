...
admit language intrinsic types, i.e. interfaces that are formulated in
terms of type sets to enforce generics constraints, as they are
discussed in Sect.~\ref{sect:type_sets}. These include the empty
interface, \code{IAnyType}.

\subsection{Extends specifier for abstract interfaces}

Abstract interface definitions must allow the programmer to declare
new abstract interfaces that are the union of \emph{multiple} simpler
ones (multiple interface inheritance). In the following example, the
...
\subsection{Short-hand notation for union of abstract interfaces}
...
functionality for user-defined types \emph{only}, i.e. its use on
...
simply called \code{T} here, and in terms of which the regular
function parameters are declared. A significant difference of generic
type parameters as compared to regular function parameters is that the
...
abstract interfaces as type sets. The following example shows the
simplest form of such a type set, by defining an interface
\code{INumeric}, for use as a generics constraint in the example of
Sect.~\ref{sect:generic_interfaces}, in order to admit for the type
parameter \code{T}, that was given there, only the (32 bits wide)
standard \code{integer} type:
\begin{lstlisting}[language=LFortran,style=boxed]
...
More useful interfaces as generics constraints can be defined by
specifying unions of types as a type set, as in the following example
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: INumeric
      integer | real(real64)
   end interface
\end{lstlisting}
that defines interface \code{INumeric} such as to admit either the
standard \code{integer} or the \code{real(real64)} type.

The semantics of a type set construct is that it implicitly defines a
\emph{set of function signatures}, namely the signatures of the
intersecting (common) set of all the operations/intrinsic functions
that work with the types that are listed in the type set. This can
also be restated, by saying that each type of the type set
\emph{implements} the interface that contains this type set. The
\code{complex} type, for instance, does not implement the interface
\code{INumeric}, as it was given in this section, because it does not
support, i.e. ``implement'', the relational operators \code{<} and
\code{>} that are implemented by the \code{integer} and
\code{real(real64)} types.
 
Notice also, that interfaces that are formulated in terms of type sets
are merely intended for use as generics constraints. In particular,
they cannot be used in variable declarations that involve the
\code{class} specifier (Sect.~\ref{sect:class_specifier_constraints}),
and they are not intended to be implemented by derived
(i.e. user-defined) types (Since any new intrinsic function that would
have to be added to the language for some intrinsic type, would change
the set of methods of all type sets in which this type appears. Thus
breaking user-defined types that would implement interfaces which
contain these type sets).

\subsubsection{Kind parameters}

As a further example, an \code{INumeric} interface that is potentially
even more useful as a generics constraint could be coded as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: INumeric
...
A particularly important case is the empty interface, that matches
all types, because all types have at least zero methods:
\begin{lstlisting}[language=LFortran,style=boxed]
...
programmer to employ a shorter notation for declaring type
constraints, than having to define a separate interface, like
\code{INumeric} above, and to then use it as in
...
defines an abstract interface implicitly, to be used as a type
...
The language should ideally supply a collection of predefined,
commonly used generic constraints in the form of abstract interfaces
that are contained in a language intrinsic module, tentatively called
\code{generic\_constraints} here. The actual code of these interfaces
could then employ the ``inter\-faces-as-type-sets'' syntax that was
described above, or any other suitable means. The list of such
predefined interfaces should include
\begin{itemize}
\item
  an empty interface of the name \code{IAnyType} (as shown above),
\item
  a general interface for numeric operations, \code{INumeric}, but also
\item
  separate interfaces for all the language's numeric and relational
  operators.
\end{itemize}
The latter interfaces are required for the user to formulate selective
generics constraints for intrinsic types. But also in order to enable
the programmer to implement these operators himself for any new types
where this would be desirable, or where these operators are not
supported by default.

Language provided interfaces could then be imported from user code
...
\end{lstlisting}
They could be used as constraints in function and derived type
implementations, or as constraints in other interfaces, like
\code{ISum} here, that requires the functionality of \code{INumeric}.
...
initialization by transforming the (typeless) zero constant, \code{0},
into the corresponding constant of type \code{T}, i.e. by simply
writing \code{s = T(0)}.
...
\end{lstlisting}
Generic subroutines can be coded completely analogously. To actually
use the \code{sum} generic function, one simply needs to call it with
arguments of (different) ``numeric'' types (in its regular arguments
list), e.g. as in the two following examples:
\begin{lstlisting}[language=LFortran,style=boxed]
...
\end{lstlisting}
Such manual instantiation of generic procedures can be useful
for combination with \code{associate} statements
\begin{lstlisting}[language=LFortran,style=boxed]
  associate( sumf => sum{real(real64)} )
     total1 = sumf([1.d0,2.d0,3.d0,4.d0,5.d0])
     total2 = sumf([2.d0,4.d0,6.d0,8.d0])
  end associate
\end{lstlisting}
or for use with procedure pointers, as in the following example:
\begin{lstlisting}[language=LFortran,style=boxed]
...
instantiated with particular argument types substituting the generic
...
parameter \code{U} conforms to \code{IAnyType}. Notice, that in order
to maintain consistency between the type of the \code{elements} array,
and any new item that we wish to append to it, we must force the
equally named \code{item} argument of method \code{append} to be of
\code{type(U)} as well, as it is shown in this method's actual
...
method \code{append}'s implementation, the compiler will then
implicitly assign type \code{U} to the type alias \code{Element}, in
order to conform to interface \code{IAppendable}'s definition.
...
decoupling it and making it operate on multiple data types, thus
allowing for a maximum of code reuse. Notice, also, that (as in the
functional Fortran implementation of Listing~\ref{lst:FuncFortran}) not a
single manual instantiation of generics is necessary anywhere in the
code. The Fortran version described here is therefore as clean as the
Go implementation of Listing~\ref{lst:OOGo} with respect to dependency
management, and it is as easy to use, and to read and understand, as
the Swift implementation of Listing~\ref{lst:OOSwift}.
...
the types of the regular arguments:
\begin{lstlisting}[language=LFortran,style=boxed]
  avs = Averager{SimpleSum}(drv = SimpleSum())
  avp = Averager{PaiwiseSum{SimpleSum}}(drv = PairwiseSum(other = SimpleSum()))
\end{lstlisting}
