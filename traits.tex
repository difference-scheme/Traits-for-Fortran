...
type sets. The following example shows the simplest form of such a
type set, by defining an interface \code{INumeric}, for use as a
generics constraint in the example of
...
More useful interfaces for constraining generic type parameters can be
obtained by specifying unions of member types as a type set, as in
...
that work with all the member types of the type set. This can also be
restated, by saying that a type \code{T} \emph{implements} an
interface consisting of such a type set, if (and only if) it is a
member of this type set. The \code{complex} type, for instance, is not
a member of interface \code{INumeric}'s type set (as it is given in
this section), because it does not appear anywhere within this
interface. Hence, \code{complex} also does \emph{not} implement the
...
to support compile-time polymorphism through union (also called sum)
types \cite{Taylor_21} (which would take this design to its logical
conclusion and fill a present gap in the later to be discussed
Table~\ref{tab:dispatch}).
