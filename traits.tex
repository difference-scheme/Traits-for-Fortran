...
protocols/interfaces in that the user can implement them for a
user-defined type without having these implementations be coupled to
the type's actual definition. Hence, existing types can be made to
retroactively implement new traits, and hence be used in new settings,
even if their source code itself is inaccessible (because it is, for
instance, contained in a binary library).

These ideas were quickly absorbed by almost all other mainstream
...
\code{struct}'s) definition. Go thus implements a true ``trait
system''. Go, finally, makes it explicit in its type definition syntax
that interfaces (like \code{struct}'s) are types in their own right,
...
Like Go and Rust, Swift (furthermore) supports a true ``trait system''
in order to implement run-time and compile-time polymorphism through
...
class's own name. However, functionality for types to implement
protocols retroactively is also provided, through so-called
``\code{extension}s'', which are similar to Rust's \code{impl} blocks.
...
The use of run-time polymorphism by means of interfaces is very
similar in all the languages considered here. This includes the fact
that all these languages offer true ``trait systems'', where the
implementation of an interface by a user-defined type can be
accomplished independently from the type's definition site (which is
for instance not possible in ``classical'' OO languages, like
Java). All of these languages thereby collectively rebut Haveraaen et
al.'s critique \cite{Haveraaen_et_al_19} of Java. In fact, it is
...
first place in these modern languages, that Haveraaen et al. seem to
also (rightly) demand. Given their many similarities, we will thus
only make some final comments on the differences in these languages'
compile-time, i.e.  generics features.
...
a \code{struct}'s definition through explicit \code{impl} blocks, and
the complete control over the use of dynamic vs. static dispatch that
Rust affords the programmer. These are particular features of Rust
that, in our opinion, Fortran should borrow in some form.
...
traits, for types, and to hence support a true trait
system. Extensions fulfill the same purpose as Rust's \code{impl}
blocks, but have been adjusted here to Fortran's syntax that binds
methods to types through declarations, rather than by including the
actual implementation bodies themselves into such a block (the
...
derived type definitions. Most of the options that are allowed for
type-bound procedure declarations in derived type definitions, are
therefore also allowed for such declarations within \code{extension}s.
