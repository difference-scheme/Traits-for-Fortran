...
\abstract{Based on conclusions drawn from a survey of modern
  ...
  languages like Swift, Rust, Go, or Carbon. The discussed extensions
  allow for modern-day, traits-based, object-oriented programming, and
  fully type-checked generics. The latter support seamlessly both the
  procedural, functional, and object-oriented programming styles, and
  they largely get by without requiring manual instantiation by the
  user. The presented design can also be naturally extended in the
  future to support rank-genericity for arrays, and compile-time
  polymorphic union/sum types. The new capabilities are expected to
  transform the way both Fortran applications and libraries will be
  written in the future. Decoupled software plugin architectures with
  enormously improved source code flexibility, reusability,
  maintainability, and reliability will become possible, without any
  need for run-time type inspections, and without any loss in
...
\end{lstlisting}
That is, objects that adopt or implement the \code{IBasicMath}
interface (i.e. conform to it), can also be used in settings that
require conformance to either the \code{IAddition}, or
\code{ISubtraction} interfaces.

The next example finally shows an interface, \code{IExtendedMath},
that not only inherits, as before, the procedure signatures contained
in the \code{IAddition} and \code{ISubtraction} interfaces, but also
adds to new procedure signatures (\code{mul} and \code{div}) of its
own:
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface, extends(IAddition,ISubtraction) :: IExtendedMath
      subroutine mul(self,b)
         import; implicit none
         class(IExtendedMath), intent(inout) :: self
         real,                 intent(in)    :: b
      end subroutine mul
      subroutine div(self,b)
         import; implicit none
         class(IExtendedMath), intent(inout) :: self
         real,                 intent(in)    :: b
      end subroutine div
   end interface IExtendedMath
\end{lstlisting}
...
in terms of them. Either directly, i.e. as objects of such
interfaces/traits in run-time polymorphism, as demonstrated in this
section. Or indirectly, as constraints on generic type parameters in
compile-time polymorphism (see Sect.~\ref{sect:generic_parameters} for
examples of the latter).

\subsection{Class specifier with traits}
...
abstract interfaces/traits, like in the following declaration of a
procedure argument (taken from Sect.~\ref{sect:interface_defs})
\begin{lstlisting}[language=LFortran,style=boxed]
  class(IAddition), intent(inout) :: self
\end{lstlisting}
...
\subsection{Class specifier with trait combinations}

There are often cases where some (trait) object needs to be declared
that commands over the combined functionality of two (or more)
interfaces/traits, but where one would not like to go through the
labor of explicitly setting up a separate derived interface, like
\code{IBasicMath} of Sect.~\ref{sect:extends_spec}. In such cases, it
should be possible to express the object's declaration as follows
\begin{lstlisting}[language=LFortran,style=boxed]
   class(IAddition,ISubtraction), allocatable :: addsub
\end{lstlisting}
instead of having to explicitly derive an interface \code{IBasicMath}
from \code{IAddition} and \code{ISubtraction}, as in
Sect.~\ref{sect:extends_spec}, and to then use it as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
   class(IBasicMath), allocatable :: addsub
\end{lstlisting}


\section{Implementing traits}
...
\subsection{\code{Implements} type-attribute}
\label{sect:implements_spec}

User-defined (that is derived) types can be made to conform to an
interface by introducing an \code{implements} attribute to derived
...
\end{lstlisting}
Notice, how \code{BasicMath}, by virtue of being an implementer of
\code{IBasicMath}, is now also an implementer (or subtype) of
\code{IAddition}, and \code{ISubtraction}. Hence, \code{BasicMath} can
be used in client code that requires conformance to either one of the
interfaces \code{IBasicMath}, \code{IAddition}, or
\code{ISubtraction}.
...
that it adopts. However, any non-abstract type that \code{extends}
this abstract type through subclassing (i.e. implementation
inheritance) must then provide a full implementation.
...
from the fusion of the previous two, as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
module enhanced
...
\end{lstlisting}
Or we could have employed two separate \code{implements} statements, in
order to conform to one interface at a time, like so:
\begin{lstlisting}[language=LFortran,style=boxed]
module enhanced
...
since the new \code{implements} attribute allows for inheritance of
...
discussing further enhancements that are required in order to support
...
Sect.~\ref{sect:generic_procedures} would, for instance, take the form
\begin{lstlisting}[language=LFortran,style=boxed]
...
\end{lstlisting}
whereas the corresponding manual instantiation case would be written as:
\begin{lstlisting}[language=LFortran,style=boxed]
  type(SimpleSum) :: simple
...
interfaces, as it is possible in both the Swift and Rust languages.
...
(cf. Sect.~\ref{sect:Go_encapsulated}). Associated types therefore fit
seamlessly into the philosophy of the present generics design not to
burden the user.
...
application of orthogonal language design. That is, new capabilities
are provided through the simple extension of existing Fortran
features, and their mutual interaction, rather than the addition of
(complex) new constructs. Indeed, only in one single case (the
\code{implements} statement of Sect.~\ref{sect:implements_stmt}) did
we find it necessary to introduce a new language feature.

The end result is a language that is both significantly easier to use,
and much more powerful, than that of competing approaches which prefer
to abandon the proven philosophy of orthogonal design. The presented
extensions are fully backwards compatible with the present Fortran
language, and (in stark contrast to other approaches) allow for a
consistent use of generics not just in procedural and functional, but
\emph{also} in OO programming settings, including the support of
static method dispatch (which can improve the performance of
