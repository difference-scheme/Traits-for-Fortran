...
More useful interfaces can be defined by specifying unions of
types as a type set, as in the following example
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: INumeric
      integer | real(real64)
   end interface
...
As a further example, a potentially even more useful \code{INumeric}
interface could be coded as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: INumeric
...
\code{complex} types, admitted by the language, in a single
...
\end{lstlisting}
The notation within the generic type parameter list defines an
abstract interface implicitly, to be used as a type constraint for
type \code{T}. In this particular case, to admit only the default
\code{integer}, or \code{real(real64)} types, for \code{T}, as
discussed above.
...
arguments that are of exactly the same generic types, over which its
...
implementation. In order to be able to do this enforcement without
...
functional Fortran implementation, i.e.
Listing~\ref{lst:FuncFortran}) not a single manual instantiation of
