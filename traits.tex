...
\lstdefinelanguage{LFortran}[]{Fortran}{
  morekeywords={abstract,type,class,extends,
    implements,extension,import,pass,annex}
}
...
thereby enable a uniform management of dependendencies on both
...
polymorphism via subtyping, or as constraints on generic type
parameters in compile-time polymorphism (see Sect.~\ref{} for examples
of the latter).

In particular, in order to use abstract interfaces to support run-time
...
examples above), or that they be arguments to a procedure (as in the
example of Sect.~\ref{sect:interface_defs}). This is in full
accordance with how variables, that make use of run-time polymorphism
through implementation inheritance (i.e. subclassing), are declared in
the present Fortran standard. The proposed additions are therefore
backwards compatible with the functionality that is already available
...
interface \code{IBasicMath} inherits the procedure signatures
contained in both the interfaces \code{IAddition}, and
\code{ISubtraction}, making \code{IBasicMath} at the same time a
\emph{subtype} of both these simpler interfaces. That is, objects that
adopt or implement the \code{IBasicMath} interface (i.e. conform to
it), can also be used in settings that require conformance to either
the \code{IAddition}, or \code{ISubtraction} interfaces.
\begin{lstlisting}[language=LFortran,style=boxed]
...
\emph{other types} to do the same, regardless of whether such types
are user-defined or intrinsic to the language.
...
of being an implementor, or ``descendant'', of \code{IBasicMath}, is
now also a descendant of \code{IAddition}, and
\code{ISubtraction}. Hence, \code{BasicMath} can be used in client
code that requires conformance to either one of the interfaces
\code{IBasicMath}, \code{IAddition}, or \code{ISubtraction}.
...
Rust) to make \emph{any} type implement new interfaces, regardless of
where its original type definition is located, or whether the type is
a user-defined or a language intrinsic one. This means that methods
should also be allowed for \emph{intrinsic} types. The \code{annex}
feature that is described below is aimed at accomplishing these
capabilities.

Notice, that such a type \code{annex} has \emph{no} relation to
subclassing, i.e. one derived type being extended into another through
(rigid) implementation inheritance. Rather, this is a feature that adds
new capabilities to a single, \emph{given} type. The feature is
modelled after the ``\code{extension}'' feature of Swift, where it is
used to enable retroactive implementation of new methods, additional
constructors, and especially new abstract interfaces for types, in
order to dynamically change an \emph{interface inheritance hierarchy},
and achieve utmost code flexibility.

Swift's \code{extension} blocks fulfill essentially the same purpose
as Rust's \code{impl} blocks in this respect. They have been somewhat
simplified (for a first implementation), and adjusted here to
...
this type some additional functionality. This can be done using a type
\code{annex} as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
module enhanced
...
described by two interfaces called \code{IMultiplication} and
...
of the required functionality, by acknowledging this with an
\code{implements} specification for these two interfaces as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
   ...
   use enhanced, only: BasicMath
  
   annex, implements(IMultiplication,IDivision) :: BasicMath
   end annex BasicMath
...
simultaneously, splitting the \code{annex} statements into two, to
conform to one interface at a time, like so:
\begin{lstlisting}[language=LFortran,style=boxed]
   ...
   use basic, only: BasicMath
  
   annex, implements(IMultiplication) :: BasicMath
      procedure, public, pass(self) :: mul
   end annex BasicMath

   annex, implements(IDivision) :: BasicMath
      procedure, public, pass(self) :: div
   end annex BasicMath
   ...
\end{lstlisting}
The result would have been the same. Such splitting of \code{annex}
statements can be useful to improve readability, if different
interfaces contain multiple procedure signatures, that would all have
to be implemented. These two statements (together with the required
implementations), could then even be distributed among different
modules and files.

\section{Interoperability with implementation inheritance}
...
adds two further methods of its own (namely \code{method3} and
...
discussing some further enhancements that are required in order to
further support compile-time polymorphism, i.e. generics.
...
concepts''. Both, the type parameter and its constraint, are part of a
generic type parameter list that is enclosed in curly braces, and that
follows immediately behind the procedure's name. Notice that, since
\code{T} is a meta-type, the syntax used above, that deviates slightly
from how Fortran's usual function arguments are declared, appears
justified as it reflects that, in type parameters, one is dealing with
different entities.
...
abstract interfaces as type sets. The following example
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: INumeric
      integer | real(real64)
   end interface
\end{lstlisting}
defines the interface \code{INumeric}, that was used as a generics
constraint in Sect.~\ref{sect:generic_interfaces}, in order to admit
for the type parameter \code{T} (that was given there) only the
standard \code{integer} or \code{real(real64)} types. The semantics of
such a construct is that it implicitly defines a \emph{set of function
signatures}, namely the signatures of the intersecting (common) set of
all the operations/functions for which the listed intrinsic types
...
than having to define a separate interface, like \code{INumeric}
above. Like in the following modification of interface \code{ISum}'s
original declaration, that was given in
...
types, for \code{T}, as before.

\subsection{Predefined interfaces}
...
language, in a single \code{abstract interface} constraint. Such
language provided interfaces could be imported from user code through
a \code{use} statement like in the following example for
\code{INumeric}:
\begin{lstlisting}[language=LFortran,style=boxed]
module user_code
...
\end{lstlisting}
They could then be used as constraints in function and derived type
implementations, or in other interfaces, like \code{ISum} here, that
requires the functionality of \code{INumeric}.

\section{Built-in facility for conversion to generic types}

A language like Fortran, that is intended for numeric use, where
conversions between different numeric types are required quite
...
variable for summation needs to be initialized to the zero constant of
type \code{T}. In Go, it is easily possible to express this
initialization by transforming the zero constant of the \code{int}
type, \code{0}, into the corresponding constant of type \code{T}, by
simply writing \code{s = T(0)}.

If, for instance, \code{T} is then instantiated at compile time with
the \code{float64} type, the expression \code{T(0)} will simply be
...
As already mentioned above, LFortran's basic generics design should
follow Swift's, if possible, and allow both ordinary and type-bound
procedures (i.e. methods), and derived types to be given their own
...
The same algorithm implemented as a generic method \code{sum}, that is
bound to a derived type with name \code{SimpleSum} which implements
the interface \code{ISum} as it was given in
Sect.~\ref{sect:generic_interfaces}, would instead look as follows:
\begin{lstlisting}[language=LFortran,style=boxed]
...
implemented by another derived-type, named PairwiseSum:
\begin{lstlisting}[language=LFortran,style=boxed]
...
type) is not parameterized by any generic type parameter itself.

We also propose to make a further, small, but important addition:
namely to introduce the notion that a structure constructor is
implicitly defined within the same scope that holds the definition of
its derived type. Assuming that this scope is a module, then the
structure constructor will always be able to access all the components
of its derived type, even if these are declared being \code{private}
to the module's scope. Hence, these components could be initialized
even by calls to the structure constructor that are being performed
from outside the module's scope, in complete analogy to how
user-defined constructors work in Fortran.

In this way, \code{private}, \code{allocatable} derived type
components could be initialized by structure constructors, which is
crucial for concise OO programming. Since such an extension would
merely add to the capabilities of the language, it would be fully
backwards compatible. The elegance of the Go and Swift code versions,
but also of the Fortran code examples that are presented in the next
chapter is largely due to the use of such enhanced structure
constructors. Lacking these, user-defined constructors would have to
be used, leading to overly complex implementations, as e.g. in the
...
present design offers a lot of room in this respect, by means of the
device of ``associated items'' or ``associated types'' as they are
implemented in both \code{trait}s and \code{protocol}s in Rust and
Swift, respectively. In addition to different array ranks and array
dimensions, also different ``length'' parameters of character
variables could be treated via this same device. But for the purpose
of a very first prototype implementation of the generics features
described here, we consider a discussion of these features to be
non-essential, and we thus defer them to a separate document.
...
\item
  ...
  to implement the interface \code{INumeric} as a type set, in order
  to express type genericity for the input array \code{x} and the
  result of the summation \code{s} in our different implementations of
  method \code{sum}.
\item
  As in the corresponding Go version, \code{INumeric} is defined by
  the user himself. There is thus no need for an external dependency.
\item
...
Moreover, it is as easy to read and understand as the Swift
implementation.
...
\section{Encapsulated version with static dispatch of \code{sum} methods}

One of the greatest benefits of the present design is that methods in
OO programming can be made polymorphic, while still being dispatched
...
compiler should be able to infer the correct type parameters that are
...
generally preferrable over that of
Listing~\ref{lst:staticFortran}. The use of numerous generic
...
\section{Version with static object declarations}

In the present simple example, and taking Listing~\ref{lst:staticFortran}
as a baseline, it is actually possible to even get rid of some of the
...
Correspondingly, the object instantiations that are to be carried out
from the main program need to be changed from
\begin{lstlisting}[language=LFortran,style=boxed]
  avs = Averager(drv = SimpleSum())
  avp = Averager(drv = PairwiseSum(other = SimpleSum()))
\end{lstlisting}
into the following two calls of \code{Averager}'s parameterized
structure constructor, which must now be provided with the types
\code{SimpleSum}, and \code{PairwiseSum} as generic type parameters:
\begin{lstlisting}[language=LFortran,style=boxed]
