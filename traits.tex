...
trait for a user-owned type, is also possible. Only the case of
...
concepts''. The code in the following Listing~\ref{lst:OORust} is in
its outline quite similar to the Go code version. However, see the
following notes for the differences.

\begin{itemize}
\item
  Rust uses angled brackets to indicate generic parameter lists.
\item
  Use of a \code{Num} trait was necessary to enable numeric operations
  on generic types, which leads to dependency on external library
  code. Nevertheless, conversion to generic types wasn't possible.
  Hence, some type conversions had to be moved to the main program.
\item
...
\item
  ...
  them (this is the Rust equivalent to using \code{allocatable}
  polymorphic objects in Fortran).
\end{itemize}
...
declaration blocks, rather than by including the actual implementation
