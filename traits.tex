...
It is often the case, that a method signature, that is contained
within an abstract interface, needs to declare arguments of a yet
unknown type. This type could, e.g., be a generic parameter over
which a derived type, that is implementing the interface, might need
to be parameterized.
...
to be of \code{type(U)} as well, as it is shown in this method's
...
(cf. Sect.~\ref{sect:trait_objects}) to avoid rigid dependencies on
user-defined implementation types, and generics to admit different
language intrinsic types as arguments to (and results of) methods, thus
allowing for a maximum of code reuse. Notice, in particular, how the
...
already the case in the functional implementation of
...
and much more powerful, than the result of approaches that prefer to
...
largely without requiring manual instantiations of generics, and
