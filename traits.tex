...
definition. Restrictions in Go are that language intrinsic types
cannot have methods, and that methods and interfaces cannot be
...
wrappers in the latter case. Go, finally, makes it explicit in its
type definition syntax that interfaces (like \code{struct}s) are types
...
these parameters, by passing the same type arguments to them using
...
the use of generics, polymorphic methods in OO programming (OOP) can
be made to use static dispatch\footnote{In contrast to polymorphic
methods in traditional OOP, that make use of dynamic dispatch.}. This
will enable inlining of polymorphic methods by the compiler, to
potentially improve code performance. Listing~\ref{lst:staticFortran}
gives a \emph{minimally} changed version of the encapsulated Fortran
code of Listing~\ref{lst:OOFortran}, to effect static dispatch of the
