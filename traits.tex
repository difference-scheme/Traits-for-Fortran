...
Sect.~\ref{sect:interface_defs}, for two different data types. This
serves to enable the latters' use with both the \code{generic\_sum}
function, but also a second internal subroutine, \code{printout}, that
requires I/O functionality. A derived type with name \code{MyType}
illustrates the procedure of implementing these interfaces for a
user-defined type, while the same is also shown for the \code{real}
type, as an intrinsic type example.
...
implementing type to provide functionality for appending some items to
...
\code{deferred(item)}, for the type of appendable items, that needs
to be supplied by an actual implementation of that interface.
...
philosophy not to burden the user.
