...
admit language intrinsic types, like the empty interface,
\code{IAnyType}, or any interfaces that are formulated in terms of
type sets, as they are discussed in Sect.~\ref{sect:type_sets}.
...
simply called \code{T} here. A significant difference of generic type
...
meta-type. This (meta-type) constraint is an abstract interface name
(\code{INumeric} in the present example), that precedes the type
...
immediately behind the procedure's name. Notice that the syntax used
above, that deviates slightly from how Fortran's regular function
arguments are declared, appears justified, as it reflects that,
despite some similarities, in type parameters one is dealing with
different entities.
...
standard \code{integer} or the \code{real(real64)} type. The semantics
of these type set constructs are that they implicitly define a
