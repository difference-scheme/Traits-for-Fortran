...
\abstract{Based on conclusions drawn from a survey of modern
  ...
  open source compiler (like LFortran, LLVM Flang, or GNU Fortran),
  but at the same time comprehensive enough to already endow Fortran
  with polymorphism capabilities that equal those of modern
  programming languages like Swift, Rust, Go, or Carbon. The discussed
  extensions allow for modern-day, traits-based, object-oriented
  programming, and powerful, easy to use, fully type-checked,
  generics. The latter support seamlessly both the procedural,
  functional, and object-oriented programming styles, and they largely
  get by \emph{without} requiring manual instantiations by the
...
\emph{two} such objects are required (one for each language-intrinsic
...
bind to a type so-called ``associated functions'' which aren't
methods, i.e. which don't take in a \code{self} (passed-object dummy)
argument. A typical example for this are user-defined constructors for
\code{struct}s. See the functions that are named \code{new} in the
following code Listing~\ref{lst:OORust}, and which are called using
\code{::} syntax.
...
\item
  ...
  statements.
\item
  Rust does not supply an equivalent to Go's \code{int32 | float64}
  syntax and semantics. In order to enable numeric operations on
  generic types, Rust instead provides a \code{Num} trait via an
  external \code{num} crate (i.e. library). However, the use of this
  external dependency makes available neither the \code{AddAssign}
  (\code{+=}) operator, nor casts to generic types within generic
  routines (as they are provided in Go).
\item
  ...
  requirement to also implement an associated function, \code{new},
  that provides the needed type casting (notice that \code{Self}
  stands in here for the implementing type). The conformance of the
  \code{i32} and \code{f64} intrinsic types to the \code{INumeric}
  trait is then acknowledged via \code{impl} blocks, that also
  implement any of the still outstanding functionality of this trait
  for these two types.
\item
...
\begin{itemize}
  ...
  \item
    ...
    for numeric types.
  \item
    Swift's own \code{Numeric} protocol does \emph{not} include the
    division operation! Hence, the following code defines and uses
    a custom \code{INumeric} protocol, that extends the former, to
    require any adopting types to implement also the division
    operator.
  \item
    Swift's \code{Int32} and \code{Float64} types already implement,
    individually, both the functionality of Swift's own \code{Numeric}
    protocol, and the division operation. It therefore suffices to
    simply acknowledge, through empty \code{extension} statements,
    that they also conform to the combined functionality of the
    new \code{INumeric} protocol.
  \item
  ...
  \item    
    ...
    Swift by calling an appropriate initiali\-zer of this type. The call
    syntax is a bit peculiar, i.e. Go's \code{T(0)} is written as
    \code{T(exactly:0)!} (making use of both the initializer's
    mandatory keyword, \code{exactly}, and the \code{!}  operator to
    unwrap the returned optional type).
\end{itemize}
...
named abstract interfaces. Moreover, this should be possible even
...
provision of implementations should be possible regardless of any
original type definition site\footnote{Exempting only the case of
abstract derived types, as it is discussed below.}.

The \code{implements} statement, that is described in this section,
...
\code{impl} blocks. They have been somewhat simplified here (for a
...
The syntax of the feature is similar to that of the \code{contains}
sections of derived type definitions. That is, \code{implements}
statements make use of type-bound procedure declarations. The latter
employ mostly the same attributes (i.e. \code{public}, \code{private},
\code{pass[(arg-name)]}, \code{nopass}, and \code{non\_overridable})
which are also allowed in derived type definitions. ``Generic''
type-bound procedure declarations (as the Fortran standard calls them)
can be used as well. The main difference to derived type definitions
is that the \code{implements} statement cannot be used with
\code{abstract} derived types, and that it hence also does not accept
\code{deferred} type-bound procedure declarations. If interoperability
with \code{abstract} types is required, the later to be discussed
\code{implements} derived type attribute (cf.
Sect.~\ref{sect:derived_types}), that has none of these restrictions,
can be used instead.

As a final disclaimer, notice that the \code{implements}
...
\subsection{Binding methods to a type}
\label{sect:adding_methods}

The simplest use of the \code{implements} statement is to bind some new
methods to a type. The following example shows how to bind two methods,
...
Assume now, that the purpose of binding the previous two methods to
the \code{Calculable} type was to actually make this type compatible
with settings where conformance to the \code{ICalculable} interface of
Sect.~\ref{sect:extends_spec} is required. So far, we have provided
both the binding of the required methods, and their implementations,
but we haven't made \code{Calculable} pluggable yet into code that is
written in terms of the \code{ICalculable} interface. To fix this, we
can simply acknowledge (even from a different module, as in the
following example), that the \code{Calculable} type already has all of
the required functionality to implement the \code{ICalculable}
interface:
\begin{lstlisting}[language=LFortran,style=boxed]
module enhanced
   ...  
...
a (comma-separated) list of interface names to the \code{implements}
...
implemented after their necessary method implementations were already
...
(e.g. the \code{real(real64)} type in the above example), then this
...
\code{real64} constant in the above \code{printx} example program), or
...
Sects.~\ref{sect:generic_interfaces} and \ref{sect:generic_parameters}
...
conform to multiple interfaces, but where one would like to avoid
having to code some (otherwise unneeded) intermediary interface, that
derives from these. In such cases, it should be possible to express an
object's declaration more directly, by providing a list of interface
...
\code{IAddable} and \code{ISubtractable} traits.

\section{Derived types}
\label{sect:derived_types}

For reasons of both regularity in the language, and full
interoperability with Fortran's legacy (i.e. present) OO model, it
should be possible to implement interfaces also directly from within
derived type definitions. This can be accomplished by allowing an
\code{implements} \emph{attribute} within such definitions, that -- in
contrast to the previously discussed \code{implements} statement --
features also interoperability with \code{abstract} derived
types. Notice, however, that by its nature of being bound to derived
type definitions, the former attribute (necessarily) lacks the latter
statement's retroactive implementation capabilities, that are
important for fully exploiting the flexibility that the modern
traits-based OO programming model offers.

\subsection{Implements and sealed attributes}
\label{sect:sealed_types}

The following code shows how the ``one fell swoop'' example of
...
\subsection{Implements and extends attributes}
...
subclassing). However, the multiple interface inheritance
(or subtyping) features, that are provided by the \code{implements}
...
extension capabilities which are already present in the language. For
instance, code examples like the following are possible:
\begin{lstlisting}[language=LFortran,style=boxed]
   type :: Parent
   contains  
      procedure :: method1
...
Since the \code{implements} derived type attribute allows for
inheritance of \emph{multiple} interfaces, this also fixes present
...
\subsection{Implements attribute and abstract derived types}
\label{sect:abstract_types}

The examples of the \code{implements} derived type attribute, that
were presented in the previous two Sects.~\ref{sect:sealed_types} and
\ref{sect:extended_types}, could have been coded equally well using
...
namely implementation of interfaces even if the implementing derived
type is \code{abstract}. In such cases, the implementing type is
allowed to provide an only partial implementation of the interface(s)
that it adopts. Any non-abstract type that \code{extends} this
\code{abstract} type through type extension must, though, provide a
full implementation.

For instance, it is valid for the following \code{abstract} type,
which is named \code{Base},
\begin{lstlisting}[language=LFortran,style=boxed]
type, abstract, implements(Interface) :: Base
contains
   ! declarations for all but one method of Interface
   ...
end type Base
! method implementations
...
\end{lstlisting}
to implement all but one of the methods of the specific
\code{Interface} that it adopts. On the other hand, the following type
\begin{lstlisting}[language=LFortran,style=boxed]
type, extends(Base) :: Extended
contains
   ! declaration for the remaining method of Interface
   ... 
end type Extended
! method implementation
...
\end{lstlisting}
that extends the \code{Base} type, must then implement the
\code{Interface}'s remaining method. Thus, the \code{implements}
attribute makes interface inheritance interoperable even with
\code{abstract} derived types and their type extensions (and hence
with present Fortran's full OO model), in a manner that is equivalent
to how such interoperability works in the Java language
\cite{JavaTutorial}.

Notice, however, that if the programmer decides to forego the use of
\code{abstract} derived types altogether, then the previously
discussed \code{implements} \emph{statement} offers not only all of
the capabilities of the \code{implements} attribute, but also adds to
these the possibility of retroactive implementation. Since the thus
resulting programming style is a much more flexible, complete,
modern-day alternative to both \code{abstract} types and type
extension, our recommendation would be to use the interoperability
features that are described in Sect.~\ref{sect:extended_types} and the
present Sect.~\ref{sect:abstract_types}, only within legacy
applications, and to otherwise avoid the use of both \code{abstract}
types and type extension in new code.
...
the interface \code{INumeric} (as it is given above) because they are
member types of its type set. In contrast, Fortran's various
\code{complex} types do not implement this interface, because they do
...
required for conformance to this interface, because these operators
...
for generics, than having to explicitly define an interface of type
...
The facilities that are described in this section are flexible enough
for the user to be able to construct frequently required generics
constraints himself, the way he needs them. Nevertheless, the language
should ideally also supply a collection of predefined, commonly used
generics constraints in the form of abstract interfaces that are
contained in a language-intrinsic module, tentatively called
...
is named \code{other}. As it is indicated by the type constraint on
...
as \code{alloctable}s and their instantiation using chaining of
constructor calls, was kept the same in order to demonstrate that
...
As in corresponding Swift and Rust implementations of the standard
test problem, that are not fully reproduced here (see the accompanying
\code{Code} directory), the instantiation (in the main program) of the
objects \code{other} and \code{drv} through constructor calls, has the
benefit that the compiler can infer the correct type arguments, that
are required to automatically instantiate all the involved generic
derived types. Hence (and similarly to both the functional and OO code
...
generics constraints that, in the form of the \code{INumeric}
interface, were formulated as type set unions of intrinsic
types. While this is a very frequently occuring use case, it is of
course possible to formulate generics constraints in a more general
manner, through explicit specification of procedure signatures in
abstract interfaces. One such example was already given in the form of
the \code{ISum} interface, that was used as a generics constraint for
derived types in Listing~\ref{lst:staticFortran}.
...
This enables the use of \emph{both} these types with the
...
\begin{abstract}
  ...
  design, but also for its ease of use. Robert also kindly provided
  the original code version from which Listing~\ref{lst:polyfuncGo}
  was derived, and answered the many questions we had regarding Go's
  generics. We are also grateful to everyone who provided feedback to
  our work, like Magne Haveraaen and Brad Richardson, whose comments
