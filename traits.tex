...
examples above), or that they be arguments to a procedure (as in
...
of being a ``descendant'' of \code{IBasicMath}, is now also a
descendant of \code{IAddition}, and \code{ISubtraction}. Hence,
...
parameter list that is enclosed in curly braces, and that follows
...
Constraints for, e.g., different ``length'' parameters for character
variables could be handled in the same fashion. The following
interface could, for the lack of a better example, be used as a
generics type constraint, to only admit \code{character} variables
with a length of either 4 or 8 characters:
\begin{lstlisting}[language=LFortran,style=boxed]
...
than having to define a separate interface like \code{IPrintable}, or
\code{INumeric} above. Like in the following modification of interface
\code{ISum}'s original declaration, that was given in
...
previously declared (using the \code{class} specifier) as
\code{allocatable} run-time polymorphic objects that were to be
