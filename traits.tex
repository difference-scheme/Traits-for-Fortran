...
\abstract{Based on conclusions drawn from a survey of modern
  ...
  Go. The discussed new features are expected to transform the way
...
definition. Go makes it explicit in its syntax that interfaces (like
\code{struct}s) are types in their own right, and that hence
polymorphic variables (i.e. objects) can be declared in terms of them.
...
interfaces, like in the following two examples:
\begin{lstlisting}[language=LFortran,style=boxed]
...
present example), will make use of late binding. That is, their calls
...
available in the present language. See also Sect.~\ref{sect:type_sets}
for further information on the \code{class} specifier when used with
abstract interfaces.
...
\subsection{Type sets: Interfaces as generics constraints}
...
example, in which the same interface \code{ISum} of
...
We also propose to make a further, small, but very important addition
to structure constructors: namely to introduce the notion that a
...
solved by making use of separate generic type parameters in both the
derived type \emph{and} its method, and then enforcing the consistency
of these parameters, by passing the same type arguments to them using
manual instantiation. A better solution, though, that does not require
any manual instantiation of generics, can be obtained by allowing
``associated types'' to be declared within abstract interfaces, as it
is possible in both the Rust and Swift languages.
...
depends on implementations. The \emph{entire} rest of the code is
decoupled, i.e. it depends merely on abstract interfaces (see the
\code{use} statements in the above modules). The features described in
this document have enabled us to avoid rigidity in the program, by
both decoupling it and making it operate on multiple intrinsic data
types, thus allowing for a maximum of code reuse.

Notice, that all the abstract interfaces that are required for this
purpose are defined by the user himself (without any need to depend on
external libraries), and that not a single manual instantiation of
generics is necessary anywhere in the code (as it was already the case
in the functional implementation of
Listing~\ref{lst:Func1Fortran}). The encapsulated Fortran version
...
objects, in order to effect run-time polymorphism and dynamic dispatch
of their methods. See also Table~\ref{tab:dispatch} for a summary of
rules regarding method dispatch.
...
The Fortran extensions, that were described in this document for both
modern-day object-oriented and generic programming, resulted from
consistent application of orthogonal language design. Wherever
possible, simple extension of existing Fortran features was preferred
over the introduction of new ones. Indeed, only in one single case
...
and much more powerful than designs that prefer to forego the proven
philosophy of orthogonal design. The presented language extensions are
fully backwards compatible, and they allow for a consistent use of
generics not just in functional but also in OO programming (OOP)
settings, enabling the programmer to even use static dispatch in OOP,
for achieving enhanced code performance of polymorphic methods via
inlining by the compiler.

Taken together, these capabilities will allow the Fortran programmer
to manage source code dependencies on both language intrinsic and
user-defined types uniformly in his applications, and to thereby write
unprecedented modular code that is on par with the most modern
languages, without suffering any losses in computational
performance. The present design achieves all of this largely without
requiring any manual instantiations of generics, and it furthermore
leaves significant room for future language growth, like support of
rank genericity, and compile time polymorphism via union (sum) types.
