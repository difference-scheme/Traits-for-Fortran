...
\lstdefinelanguage{LFortran}[]{Fortran}{
  morekeywords={abstract,type,implements,initial,extension}
}
...
definition. Hence, existing types can be made to retroactively
implement new traits, and hence be used in new settings (with some
restrictions though on user ownership of either the traits or the
types). In Rust, traits can, for instance, be implemented by the
programer for both user-defined and language intrinsic types.

Rust's main ideas were quickly absorbed by almost all other mainstream
...
Instead, a type is implicitly assumed to implement an interface
whenever it provides implementations of all the interface's function
signatures. This way of implementing interfaces requires only an
object reference of the type to be passed to its methods (by means of
a separate parameter list, in front of a method's actual name). It is
otherwise decoupled from the type's definition (i.e. the ADT's
\code{struct} definition). A limitation in Go is that new methods and
interfaces cannot be directly implemented for an existing type that
is defined in another package (i.e. user defined wrappers are
required for this purpose). Go, finally, makes it explicit in its type
definition syntax that interfaces (like \code{struct}'s) are types in
their own right, and that hence polymorphic variables (i.e. objects)
can be declared in terms of them.
...
through polymorphic interfaces, which Rust calls ``traits''. Rust was
the first mainstream language that came with a trait system. In
...
trait, like e.g.  user-defined constructors for \code{struct}s (the
functions named ``\code{new}'' in the following code
Listing~\ref{lst:OORust}).

In contrast to Go, Rust allows the programmer to retroactively
implement traits for types that are located in external libraries
(called ``crates'' in Rust), as long as the traits themselves are
defined in the programmer's crate. The reverse, namely implementing an
external trait for a user-owned type, is also possible. Only the case
of implementing an external trait for an external type is not allowed
(the so called ``orphan rule''), and requires the use of wrappers.
...
to implement run-time and compile-time polymorphism through
...
definition. The \code{:} operator followed by one or more interface
names must be supplied for this purpose after the structure's or
class's own name. A very powerful facility for types to implement
...
Rust, and in some aspects even superior. For instance, all these
languages allow for the parameterization of functions and structures
(and in Swift's case also classes). Where Swift differs from both Go
an Rust, is that it also allows for parameterized
...
functionality. This has some interesting, postive consequences for the
...
\section{Conclusions from the different implementations}
...
difference appears to be that Go has some stricter limitations with
regards to package ownership when it comes to retroactively
implementing interfaces for existing types. Whereas Rust (with some
restrictions) and even more so Swift allow the implementation of an
...
\subsection{Conclusions on Rust}
...
\subsection{Conclusions on Swift}
...
The \code{extension}s described below are a feature that is modeled
after \code{extension}s in Swift, in order to make it possible to
retroactively implement new methods, constructors, and especially
traits, for types, regardless of where the original type definition is
located. Swift's extensions fulfill essentially the same purpose as
Rust's \code{impl} blocks in this respect. They have been adjusted
here to Fortran's syntax that binds methods to types through
declarations, rather than by including the actual implementation
bodies themselves into such a block (the implementations need to be
supplied as module procedures, as is usual in Fortran). The syntax of
the feature is largely symmetric to that of derived type
definitions. Most of the options that are allowed for type-bound
procedure declarations in derived type definitions, are therefore also
allowed for such declarations within \code{extension}s.
...
either one of these latter interfaces. To fix this, we combine the
addition of our two new methods to the \code{BasicMath} type with an
implementation of these two interfaces by this type, as follows:
\begin{lstlisting}[language=LFortran,style=boxed]  
   use basic, only: BasicMath
   ...
   ...   
\end{lstlisting}
We could also split this single \code{extension} statement into
two, to achieve the same effect, like so:
\begin{lstlisting}[language=LFortran,style=boxed]  
   use basic, only: BasicMath
   ...
   ...   
\end{lstlisting}
This can be useful to improve readability, if the different interfaces
contain multiple procedure signatures. These two statements, together
with their corresponding implementations, could then even be
distributed among different modules and files.
...
parameter. Fortran generics thus support ``strong concepts''.

Both, the type parameter and its constraint, are part of a generic
type parameter list that is enclosed in curly braces, and follows
