...
\lstdefinelanguage{LFortran}[]{Fortran}{
  morekeywords={abstract,import,type,class,extends,implements,pass}
}
...
\code{int32} and \code{float64} types as arguments to all functions, and
...
Rust) to make \emph{any} type implement new interfaces retroactively,
regardless of where its original type definition is located, or
whether the type is a user-defined or a language intrinsic one. This
means that methods should also be allowed for \emph{intrinsic}
types. The \code{implements} statement that is described below is aimed
at accomplishing these capabilities.
...
for the type parameter \code{T} (that was given there) only the (32 bits
wide) standard \code{integer} or \code{real(real64)} types. The
semantics of such a construct are that it implicitly defines a
\emph{set of function signatures}, namely the signatures of the
intersecting (common) set of all the operations/functions for which
the listed intrinsic types provide implementations.
...
above. The following modification of interface \code{ISum}'s original
declaration, of Sect.~\ref{sect:generic_interfaces}, provides such an
example:
\begin{lstlisting}[language=LFortran,style=boxed]
   abstract interface :: ISum
...
\end{lstlisting}
The above notation would define an abstract interface implicitly, to
be used as a type constraint for type \code{T}. In this particular
...
initialization by transforming the zero constant,\code{0}, into the
corresponding constant of type \code{T}, i.e. by simply writing
\code{s = T(0)}.
...
access \code{PairwiseSum}'s generic parameter \code{U}. Thereby
allowing this method to make use of two independently defined generic
...
parameters of their derived types, like in the following
...
dimensions, also different \code{length} parameters of character
...
In order to illustrate how the new features, that were discussed in
the last two chapters, can be used in practice, we will give in the
present chapter both functional and encapsulated (i.e. OO) Fortran
code versions of the test problem that was used in our language
survey, which are all expressed in terms of these new Fortran
features.
...
\item
  ...
  return values.
\item
  \code{INumeric} is defined by the user himself, and thus there is no
  need for an external dependency.
\item
  Conversions to generic types are done as in Go.
\end{itemize}

\lstinputlisting[language=LFortran,style=boxed,label={lst:FuncFortran},caption={Proposed functional, generic Fortran version of the array averaging example.}]{Code/Fortran/functional.f90}

The example demonstrates that using the new generics features with a
functional programming style is easy, that the syntax is economical,
and that automatic type inference by the compiler should be
...
described new generics features also in OO programming is not only
possible, but it is as easy to do as with the functional programming
style. Moreover, the equal importance that is placed in this document
on both proper compile-time and run-time polymorphic capabilities
allows for uniform dependency management of both language intrinsic
and user-defined types in Fortran, that is on par with the
capabilities of other modern languages. We believe that this will
completely transform the way Fortran applications will be written in
the future. Fully decoupled software plugin architectures with
enormously improved flexibility, maintainability, reusability, and
reliability will become possible, without any loss in computational
performance.

\subsection{Dynamic method dispatch}

Listing~\ref{lst:OOFortran} gives our Fortran version of the
encapsulated form of the test example that corresponds closest to the
code versions that were presented in Chapter~\ref{sect:survey} for all
the other languages.

\begin{itemize}
\item
  As in the encapsulated Go code version, and in order to manage all
  the source code dependencies in the problem, three interfaces are
  used: \code{INumeric}, \code{ISum}, and \code{IAverager}.
\item
  In contrast to the Go version, none of the aforementioned interfaces
  is parameterized itself, since we followed Swift's basic model of
  generics.
\item
...
above modules). The features described in this document have enabled
us to avoid rigidity in the program, by both decoupling it and making
it operate on multiple data types, thus allowing for a maximum of code
reuse.

Notice, also, that not a single manual instantiation of generics is
necessary anywhere in the code. The Fortran version described here is
therefore as clean as the Go implementation with respect to dependency
management, and it is as easy to use, and to read and understand, as
...
OO programming can be made polymorphic, and still be dispatched
...
as a baseline, it is actually possible to even avoid of some of the
...
structure constructor (cf. Sect.~\ref{sect:param_constructors}), which
must now be provided with the types \code{SimpleSum}, and
\code{PairwiseSum} as generic type parameters:
\begin{lstlisting}[language=LFortran,style=boxed]
